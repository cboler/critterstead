...
function at(position: Point, modify?: (state: GameState) => void): LocalGameHost {
  ...
  state.critter.position = { ...position };
  modify?.(state);
...
describe('LocalGameHost authority and care', () => {
  it('owns a separate snapshot and rejects remote, invented, and unaffordable actions', () => {
    ...
    expect(act(host, 'critter-pip', 'upgrade')).toBe(false);
    expect(act(host, 'critter-pip', 'pet')).toBe(true);
    expect(initial.critter.bond).toBe(20);
  ...
  it('does not let repeated petting or feeding manufacture unlimited benefits', () => {
    ...
    expect(act(host, 'critter-pip', 'pet')).toBe(true);
    const bond = host.state.critter.bond;
    expect(act(host, 'critter-pip', 'pet')).toBe(false);
    expect(host.state.critter.bond).toBe(bond);
    expect(act(host, 'critter-pip', 'feed')).toBe(true);
    expect(act(host, 'critter-pip', 'feed')).toBe(false);
  ...
  it('advances a full day in the configured thirty real minutes without negative needs', () => {
    ...
    expect(host.state.critter.ageDays).toBe(19);
    expect(host.state.critter.hunger).toBeGreaterThan(35);
...
describe('progression, resources, and persistence', () => {
  it('requires observation before commands, then learns independent foraging from real harvests', () => {
    ...
    expect(knowledgeStage(host.state.critter.berryKnowledge)).toBe('Harvests on cue');
    ...
    expect(knowledgeStage(host.state.critter.berryKnowledge)).toBe('Independent forager');
    ...
    expect(host.state.critter.stamina).toBe(76);
  });
  ...
  it('requires stamina and food for work and never depletes a bush on a rejected command', () => {
    ...
    expect(host.state.critter.stamina).toBe(7);
  });
  ...
  it('sleeps into a new morning, renews berries, and preserves creature identity', () => {
    ...
    expect(host.state.critter.id).toBe('critter-pip');
    expect(host.state.critter.ageDays).toBe(19);
    expect(host.state.critter.stamina).toBe(100);
    expect(host.state.resources[0].available).toBe(true);
  });
});
...
describe('training and competitions', () => {
  it('takes three real timed cues, blocks spamming, and pays energy up front', () => {
    ...
    expect(host.state.critter.stamina).toBe(85);
    ...
    expect(loaded.state.critter.stats.speed).toBeGreaterThan(4);
  ...
  it('makes care influence practice results with identical timing', () => {
    const happy = at({ x: 3, z: 2 }, (state) => {
      state.critter.happiness = 100;
      state.critter.bond = 100;
      state.critter.hunger = 0;
    });
    const tired = at({ x: 3, z: 2 }, (state) => {
      state.critter.happiness = 20;
      state.critter.bond = 10;
      state.critter.hunger = 70;
    });
    ...
    expect(happy.state.critter.stats.speed).toBeGreaterThan(tired.state.critter.stats.speed);
  });
  ...
  it('records one rewarded competition per day and cannot create repeat race money', () => {
    ...
    finishActivity(host);
    expect(host.state.critter.competitions).toHaveLength(1);
